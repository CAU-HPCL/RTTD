...
def generate_tgv_initial(N, u_0=1.0):
    """
    Generates Taylor-Green Vortex (TGV) on GPU (CuPy).
    Domain: [0, 1] x [0, 1] periodic.
    """
    # Grid setup (endpoint=False is important for periodic spectral methods)
    x = cp.linspace(0, 1, N, endpoint=False)
    y = cp.linspace(0, 1, N, endpoint=False)
    X, Y = cp.meshgrid(x, y)
    
    # Wavenumber to fit one vortex pair in [0,1]
    k = 2 * cp.pi
    
    # TGV Formula
    # u =  sin(2pi x) cos(2pi y)
    # v = -cos(2pi x) sin(2pi y)
    u =  u_0 * cp.sin(k * X) * cp.cos(k * Y)
    v = -u_0 * cp.cos(k * X) * cp.sin(k * Y)
...
def simulation(n, Re, path, initial_fields=None):
    ...
    dt = 0.1*2.0**-(n-1)                     # finite time step
    T = 2                           # final time
...
def main():
    ...
    for Re in Re_list:
        for n in n_list:
            ...
            # The Taylor-Green Vortex (TGV) scenario
            # u_init, v_init = generate_tgv_initial(2**n, u_0=1.0)
            ...
            # path = f"2D_DNS/taylor/velocity_{n}_{Re}"
